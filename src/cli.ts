...
async function main(): Promise<void> {
  ...
  if (!clientId) missing.push('--client-id or HARDEN_CLIENT_ID');
  if (!clientSecret) missing.push('--client-secret or HARDEN_CLIENT_SECRET');
  if (!serviceId) missing.push('--service-id or HARDEN_SERVICE_ID');
  if (!oidcToken) missing.push('--oidc-token or HARDEN_OIDC_TOKEN');
  if (!commitSha) missing.push('--commit-sha or CI_COMMIT_SHA');
